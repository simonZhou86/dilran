...
parser.add_argument('--dataset', type=str, default="CUFED5_fusion",
                    help="which dataset to use, available option: CUFED5_fusion")
...
ct, mri, gt = get_common_file(target_dir)
train_ct, train_mri, train_gt, test_ct, test_mri, test_gt = load_data(ct, target_dir, config.test_num)
...
for i in t:
    ...
    for j, batch_idx in enumerate(train_loader):
        ...
        img_1_fe = model.fe(img_1_slice)
        img_2_fe = model.fe(img_2_slice)

        fused = fusion_strategy(img_1_fe, img_2_fe, strategy="FL1N")
        ...
        # compute loss
        loss = loss_function_l2(img_out, gt_slice_y)
    ...
    val_psnr, val_ssim, val_nmi, val_mi = validate(model_dir + "/current.pt")
    ...
    print("PSNR", "SSIM", "NMI", "MI")
    print(val_psnr, val_ssim, val_nmi, val_mi)
